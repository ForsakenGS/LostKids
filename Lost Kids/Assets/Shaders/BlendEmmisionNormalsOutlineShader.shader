Shader "Custom/BlendEmmisionNormalsOutlineShader" {
    Properties {
...
////// Emissive:
                ...
                float3 emissive = ((_Emmision.rgb*(_EmmisionExponent*125.0+0.0))*(_Diffuse_var.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb));
                float4 _TextureAlt_var = tex2D(_TextureAlt,TRANSFORM_TEX(i.uv0, _TextureAlt));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(i.uv0, _Noise));
                float3 finalColor = emissive + (lerp(_TextureAlt_var.rgb,_Diffuse_var.rgb,saturate((saturate((_BlendStrength+_Diffuse_var.rgb))+saturate((_BlendStrength+_Noise_var.rgb))-1.0)))*((_Diffuse_var.rgb*max(0,dot(lightDirection,normalDirection)))+(pow(max(0,dot(normalDirection,halfDirection)),exp(_Glossiness))*_Specularity))*attenuation*_LightColor0.rgb*_LightColor.rgb);
...
////// Lighting:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                o.Emission = ((_Emmision.rgb*(_EmmisionExponent*125.0+0.0))*(_Diffuse_var.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb));
